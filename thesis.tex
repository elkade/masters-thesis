...
LDA jest technik¹ automatycznego wykrywania niejawnych (latent) tematów zawartych w dokumentach. LDA reprezentuje dokumenty jako mieszanki tematów, które z kolei reprezentowane s¹ jako rozk³ady prawdopodobieñstwa na zbiorze s³ów. LDA uczone jest w sposób nienadzorowany oraz mo¿e byæ traktowane jako technika redukcji wymiarowoœci, gdy¿ dopasowuje dokumentowi sk³adaj¹cemu siê z wielu s³ów reprezentacjê z³o¿on¹ z ma³ej liczby tematów. 

Rozk³ad Dirichleta to ci¹g³y rozk³ad prawdopodobieñstwa parametryzowany przez wektor $\alpha$ $K$ dodatnich liczb rzeczywistych. Wymiar wektora $\alpha$ okreœla wymiar rozk³adu. Gêstoœæ rozk³adu Dirichleta wyra¿a siê wzorem: $ f\left(x_{1},\cdots ,x_{K-1};\alpha _{1},\cdots ,\alpha _{K}\right)={\frac {1}{\mathrm {B} (\alpha )}}\prod _{i=1}^{K}x_{i}^{\alpha _{i}-1}$, gdzie {\displaystyle {\begin{aligned}&x_{1},\cdots ,x_{K-1}>0\\&x_{1}+\cdots +x_{K-1}<1\\&x_{K}=1-x_{1}-\cdots -x_{K-1}\end{aligned}}}, a $B$ to sta³a normalizuj¹ca. Noœnikiem gêstoœci rozk³adu jest $K-1$ wymiarowy sympleks.

W LDA rozk³ad Dirichleta jest wykorzystany w celu nadania pocz¹tkowych wartoœci przynale¿noœci tematów do dokumentów oraz s³ów do tematów. Cechy rozk³adu spraiaj¹, ¿e tak dobrane pocz¹tkowe wartoœci paramentrów modelu pokrywaj¹ siê z intuicj¹, ¿e dokument pokrywa jedynie ma³y zestaw tematów, a temat zawiera najczêœciej tylko ma³y zestaw s³ów. Wykorzystanie rozk³adu Dirichleta do okreœlenia wartoœci pocz¹tkowych skutkuje w rezultacie lepszym dopasowaniem dokumentów i tematów.

%TODO sprawdziæ, czy nazwy zmiennych siê nie pokrywaj¹
Przypuœæmy, ¿e mamy zestaw dokumentów. Wybieramy ustalon¹ liczbê $K$ tematów



Wybierz mieszankê tematów dla dokumentu (zgodnie z rozk³adem Dirichleta dla ustalonej liczby $K$ tematów)

	Wybierz temat zgodnie z rozk³adem wielomianowym.
	
	U¿yj tematu do wybrania s³owa zgodnie z rozk³adem wielomianowym.
	

LDA iteruje po dokumentach staraj¹c siê znaleŸæ mieszankê tematów, które z najwiêkszym prawdopodobieñstwem wygenerowa³y zbiór s³ów dokumentu.
...
Jego ide¹ jest reprezentacja tekstów ja-
ko mieszanki tematów, definiowanych jako rozk³ady prawdopodobieñstwa na zbiorze s³ów.
Z jednej strony jest to pewna metoda redukcji wymiaru danych, natomiast z drugiej strony
LDA jest narzêdziem, które dostarcza u¿ytecznej reprezentacji danych wraz z czêsto cenn¹
interpretacj¹.


Model LDA jest przyk³adem probabilistycznego modelu graficznego, a konkretniej sieci bay-
esowskiej, który zosta³ zaproponowany do modelowania danych tekstowych w [5]. Ide¹ LDA
jest reprezentacja tekstu jako mieszanki tematów, definiowanych jako rozk³ady prawdopo-
dobieñstwa na zbiorze s³ów. W modelu zak³ada siê, ¿e ka¿de s³owo w dokumencie pochodzi
z pewnego tematu i ostatecznie staramy siê opisaæ dokument jako procentowy rozk³ad za-
wartych w nim tematów. LDA jest metod¹ uczenia bez nadzoru i mo¿na j¹ traktowaæ jako
pewien sposób redukcji wymiaru, gdy¿ staramy siê przedstawiæ zbiór tekstów, przy pomo-
cy pewnej (mniejszej ni¿ liczba tekstów) liczby tematów. Ponadto z praktycznego punktu
widzenia czêsto wa¿na jest równie¿ interpretacja wyestymowanych tematów.

Przedstawimy ideê modelu LDA na przyk³adzie. Za³ó¿my, ¿e naszymi dokumentami s¹ trzy
zdania:
• Mam gor¹czkê i katar;
• Graliœmy w siatkówkê;
• Sport to zdrowie;
gdzie pogrubienie oznacza, ¿e tylko te s³owa pozosta³y w tekstach po wstêpnym przygoto-
waniu danych. Za³ó¿my równie¿, ¿e mamy okreœlone dwa tematy, z czego w pierwszym naj-
czêstsze s³owa to „przeziêbienie”, „gor¹czka”, „grypa” i inne s³owa zwi¹zane ze zdrowiem,
natomiast w drugim s¹ to „pi³ka”, „graæ”, „sport” i inne zwi¹zane ze sportem. Wówczas
je¿eli zak³adamy, ¿e s³owa w dokumentach pochodz¹ z jakiegoœ tematu, to mo¿na intuicyjnie
oceniæ, ¿e pierwsze zdanie w 100\% bêdzie sk³adaæ siê ze s³ów z tematu „zdrowie”, drugie
w 100\% z tematu „sport”, a trzecie w 50\% z obu tematów.
W niniejszej pracy opisujemy model LDA w zastosowaniu do modelowania danych teksto-
wych. Do formalnego opisu modelu, bêdzie potrzebne kilka pojêæ, które teraz zdefiniuje-
my:
• S³owo – podstawowa jednostka danych. Zbiór wszystkich s³ów nazywamy „s³ownikiem”,
a jego licznoœæ oznaczamy przez V . Formalnie s³owo reprezentujemy jako liczbê ze zbioru
{1, ..., V }.
• Dokument – skoñczony ci¹g s³ów dowolnej d³ugoœci. Formalnie jest ci¹g liczbowy o war-
toœciach ze zbioru {1, ..., V }, który mo¿na interpretowaæ jako wektor.
• Korpus – zbiór sk³adaj¹cy siê ze skoñczonej liczby dokumentów.
• Temat – dyskretny rozk³ad prawdopodobieñstwa wymiaru V , opisuj¹cy rozk³ad na zbio-
rze s³ów. W pracy bêdziemy wymiennie u¿ywaæ sformu³owañ „rozk³ad s³ów w temacie”,
„rozk³ad tematu”

W modelu LDA wystêpuj¹ trzy parametry:
• K – liczba tematów,
• ? – wektor dodatnich liczb rzeczywistych, steruj¹cy rozk³adami tematów w dokumen-
tach,
• ? – macierz wymiaru K × V , której i-ty wiersz opisuje rozk³ad i-tego tematu.
Jedynym parametrem, którego wartoœæ trzeba zadaæ, jest K. Oznacza to, ¿e musimy a priori
za³o¿yæ, ile tematów w danych chcemy wykryæ. Wartoœci ? oraz ? mo¿na ustaliæ z góry lub
wyestymowaæ ich wartoœci, w zale¿noœci od problemu, do którego wykorzystujemy model.
Liczbê dokumentów w korpusie bêdziemy oznaczaæ jako M .
...
LDA\cite{lda} jest modelem, w którym ka¿dy dokument jest modelowany przez skoñczony zbiór tematów, z których siê sk³ada. Ka¿dy temat z kolei
...
Word2vec jest stosunkowo now¹ (2013r.) predykcyjn¹ metod¹ osadzania s³ów w przestrzeni wektorowej, opisan¹ w \cite{word2vec}. 
...
%TODO Opisaæ co to jest n-gram, word2vec ma³¹ liter¹
\subsection{FastText}
...
%TODO uporz¹dkowaæ bibliografiê, dodaæ przecinki
\bibitem[1]{handbook}
