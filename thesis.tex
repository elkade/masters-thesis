...
W~teście przyjmuję poziom istotności $\alpha=0.05$. Ostatecznie, jako wynik testu Kruskala-Wallisa otrzymuję $p=0.28371>\alpha$, na~podstawie czego stwierdzam, że brak jest przesłanek, by odrzucić hipotezę zerową --- różnica między jakością rekomendacji testowanych metod nie~jest istotna statystycznie. Oznacza to, że ani żadna z~testowanych metod semantycznych nie odstaje od~innych, ani też dotychczasowa metoda \textit{Allegro} nie jest istotnie lepsza/gorsza od~innych testowanych metod.
