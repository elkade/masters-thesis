...
\begin{abstract}%DONE
	...
	Tematyka niniejszej pracy skupia siÍ wokÛ≥ zagadnieÒ okreúlania podobieÒstwa semantycznego pomiÍdzy dokumentami tekstowymi i rekomendacji dokumentÛw podobnych do danego. SzczegÛ≥owy problem pochodzi z internetowego serwisu aukcyjnego Allegro, ktÛry posiada dzia≥ artyku≥Ûw opisujπcych produkty dostÍpne w serwisie. W dziale tym funkcjonuje system rekomendacji podobnych artyku≥Ûw tekstowych w oparciu o ich treúÊ. Celem pracy jest zbadanie moøliwoúci usprawnienia dzia≥ania istniejπcego systemu rekomandacji.
	
	W niniejszej pracy adaptujÍ dostÍpne metody okreúlania podobieÒstwa pomiÍdzy dokumentami testowymi do powyøszego problemu, wprowadzam miary umoøliwiajπce ocenÍ dzia≥ania tych metod oraz dokonujÍ analizy moøliwoúci ich wykorzystania w rzeczywistym systemie. Skupiam siÍ szczegÛlnie na metodach opartych o semantycznπ analizÍ tekstu.
\end{abstract}

\begin{otherlanguage}{english}
	\begin{abstract}%TODO przet≥umaczyÊ
...
W swojej pracy badam moøliwoúÊ uøycia istniejπcych metod semantycznej analizy tekstu w odniesieniu do opisanego problemu. Badane metody to Word2vec z wariantem Word Mover's Dictance oraz bez niego. Jest to stosunkowo nowa metoda (2013), z czego wynika moje zainteresowanie. PorÛwnujÍ rÛwnieø dzia≥anie wspomnianych wariantÛw Word2vec z dzia≥aniem starszego algorytmu Latent Dirichlet Allocation.
...
\section{Rekomendacje artyku≥Ûw tekstowych w Allegro}%TODO potrzebne przypisy
% do 'najwiÍkszπ'
%TODO przeredagowaÊ
Allegro jest najwiÍkszπ dzia≥ajπcπ na rynku polskim platformπ aukcyjnπ on-line. Posiada ponad 20 mln zarejestrowanych klientÛw. Kaødego dnia na Allegro sprzedaje siÍ ponad 870 tysiÍcy przedmiotÛw. Zatrudnia 1300 pracownikÛw.\cite{allegro} Serwis umoøliwia uøytkownikom wystawianie na sprzedaø oraz kupno przedmiotÛw poprzez mechanizm licytacji lub natychmiastowego zakupu. Allegro pobiera prowizjÍ za dokonanie sprzedaøy za swoim poúrednictwem.
...
Po to, aby zachÍciÊ uøytkownikÛw do zapoznania siÍ z treúciπ kolejnych artyku≥Ûw, zastosowany zosta≥ tu system rekomendacji przyporzπdkowujπcy danemu artyku≥owi listÍ powiπzanych artyku≥Ûw. Kryterium mÛwiπcym, czy artyku≥y sπ powiπzane jest tutaj jedynie treúÊ artyku≥Ûw a nie wczeúciejsze zachowanie uøytkownika.
...
Aktualnie w rzeczonym dziale serwisu Allegro istnieje system rekomendacyjny, ktÛry opiera siÍ o wyszukiwanie podobnych artyku≥Ûw tekstowych za pomocπ silnika elasticsearch. Metoda ta opiera siÍ na s≥owach kluczowych przypisanych do kaødego artyku≥u jako metadane. W swojej pracy staram siÍ porÛwnaÊ wyniki dzia≥ania dotychczasowej metody z metodami semantycznej analizy tekstu, ktÛre potrafiπ wykryÊ podobieÒstwo pomiÍdzy artyku≥ami bazujπc jedynie na ich treúci, a nie na do≥πczonych metadanych.
...
%
%Obecnie wykorzystywana metoda generowania rekomendacji artyku≥Ûw opiera siÍ o zapytanie do us≥ugi Elasticsearch. Elasticsearch jest popularnym silnikiem wyszukiwania tekstu opartym o indeks Lucene. Dzia≥a w architekturze rozproszonej a komunikacja z nim nastÍpuje poprzez protokÛ≥ HTTP i format JSON.
%
...
W rozdziale 2 wprowadzam do zagadnienia rekomendacji i opisujÍ wybrane metody przetwarzania jÍzyka naturalnego s≥uøπce do wyszukiwania podobieÒstw miÍdzy dokumentami tekstowymi.

NastÍpnie w rozdziale 3 dokonujÍ opisu konkretnego problemu, jakim jest generacja rekomendacji artyku≥Ûw tekstowych w serwisie Allegro. OpisujÍ dane otrzymane z serwisu oraz kolejne wstÍpnego etapy przetwarzania ich, aby nadawa≥y siÍ do zaaplikowania do nich wybranych metod.

Dalej, w rozdziale 4 opisujÍ stworzone i zastosowane pÛüniej metody ewaluacji wynikÛw.

NastÍpnie w rozdziale 5 opisujÍ proces zastosowania wybranych metod oraz porÛwnanie efektÛw ich dzia≥ania. 

Ostatecznie w rozdziale 6 dokonujÍ podsumowania przeprowadzonych badaÒ i rozwaøam kierunki dalszych prac w tej dziedzinie.

Za≥πcznik A zawiera opis narzÍdzi programistycznych i bibliotek wykorzystanych przez mnie podczas prowadzenia badaÒ.

\section{Uwagi}
%TODO elastic pisaÊ z duøej litery
...
%\subsection{}
%
%W niniejszej pracy operujÍ na statycznym zbiorze artyku≥Ûw pochodzπcych wprost z serwisu (styczeÒ 2017). Serwis Allegro dynamiczne ewoluuje, stπd czÍúÊ 
...
DokonujÍ krÛtkiego wprowadzenia do zagadnienia generowania rekomendacji, ktÛrego g≥Íboka analiza nie jest konieczna z punktu widzenia niniejszej pracy. NastÍpnie wykonujÍ chronologiczny przeglπd metod ciπg≥ej reprezentacji s≥Ûw zaczynajπc od trywialnych metod zliczania s≥Ûw (bag-of-words, tf-idf), przechodzπc przez metody wykorzystujπce koncepcjÍ tematÛw (Latent Semantic Analysis, Latent Dirichlet Allocation) i koÒczπc na g≥oúnych ostatnio metodach osadzania s≥Ûw w przestrzeni wektorowej (Word2vec, GloVe, FastText).
...
\subsection{Bag-of-words}%TODO napisaÊ øe to stara metoda
...
Alaliza rozk≥adu s≥Ûw w dokumentach tekstowych pozwala na wyodrÍbnienie podobieÒstw miÍdzy s≥owami zwiπzanych z: ich znaczeniem (podobieÒstwo tematu s≥owa), ich osadzeniem w stosunku do innych typÛw s≥Ûw czy teø ich strukturπ wewnÍtrznπ. Dwie znaczπce metody: Latent Semantic Analysis\cite{lsa} oraz kolejna omawiana przeze mnie Latent Dirichlet Allocation\cite{lda} zak≥adajπ istnienie niejawnych (latent) tematÛw, do ktÛrych moøna przydzieliÊ s≥owa wchodzπce w sk≥ad korpusu.

LSA\cite{lsa}, znane rÛwnieø jako Latent Semantic Indexing (LSI) dokonuje transformacji kaødego dokumentu w wektor d≥. $|V|$ posiadajπcy na $i$-tym miejscu wagÍ TF-IDF $i$-tego s≥owa ze s≥ownika. W ten sposÛb tworzona jest macierz: kolumny reprezentujπ dokumenty a wiersze reprezentujπ unikalne s≥owa. W celu identyfikacji istotnych cech tej macierzy dokonuje siÍ rozk≥adu wed≥ug wartoúci osobliwych (singular value decomposition, SVD), ktÛry jest technikπ redukcji wymiarowoúci. Celem uøycia SVD jest redukcja liczby wierszy macierzy oraz pozbycie siÍ szumÛw, utrzymujπc jednoczeúnie podobieÒstwa pomiÍdzy kolumnami. Ostatecznie uzyskuje siÍ macierz przynaleønoúci tematÛw do dokumentÛw, gdzie wiersze odpowiadajπce tematom moøna interpretowaÊ jako kombinacje pierwotnych wierszy-s≥Ûw o podobnym znaczeniu. Np. $\{(samochod), (ciagnik), (jezdnia)\} \to \{(1.3452 * samochod + 0.2828 * ciagnik + 0.3 * jezdnia)\}$. Wymiar uzyskiwanej macierzy jest ustalany za pomocπ hiperparamtru, ktÛry oznacza liczbÍ tematÛw. Uøywajπc uzyskanej macierzy, podobieÒstwo pomiÍdzy kolumnami-dokumentami obliczane jest wykorzystujπc odleg≥oúÊ kosinusowπ (opisanπ pÛüniej w tym rodziale).

\subsubsection{Singular Value Decomposition}
%TODO czy opisywaÊ SVD

Metoda LSA ≥agodzi problem synonimÛw poprzez scalanie podobnych s≥Ûw w jeden temat. Niweluje rÛwnieø problem homonimÛw, w≥πczajπc je czÍúciowo w sk≥ad rÛønych tematÛw. Niemniej jednak poprzez arbitralne ustalanie hiperparametru odpowiedzialnego za liczbÍ tematÛw czÍúÊ semantycznie odrÍbnych tematÛw moøe zostaÊ wch≥oniÍta przez inne lub teø rozbicie na tematy moøe byÊ zbyt ,,drobne'' nie wykorzystujπc w pe≥ni semantycznych powiπzaÒ.
...
%TODO to jest wstÍp do word embeddingu

W ostatnim czasie nastπpi≥ gwa≥towny rozwÛj i niewπtpliwy sukces metod ,,word embedding'' - osadzania s≥Ûw w przestrzeni wektorowej przy pomocy uczenia nienadzorowanego.


%TODO The distributional hypothesis in linguistics is derived from the semantic theory of language usage, i.e. words that are used and occur in the same contexts tend to purport similar meanings. Harris, Z. (1954). "Distributional structure". Word. 10 (23): 146ñ162. Firth, J.R. (1957). "A synopsis of linguistic theory 1930-1955". Studies in Linguistic Analysis. Oxford: Philological Society: 1ñ32. Reprinted in F.R. Palmer, ed. (1968). Selected Papers of J.R. Firth 1952-1959. London: Longman.
...
S≥owo jÍzyka naturalnego samo w sobie jest tylko pewnym symbolem, tokenem - nie da siÍ wprost zastosowaÊ dla niego øadnych matematycznych metod, ktÛre pozwoli≥yby odwzorowaÊ semantyczne zaleønoúci pomiÍdzy s≥owami i wykorzystaÊ je do pewnych wyøszych celÛw. Medoty osadzania s≥Ûw pozwalajπ na wektorowπ reprezentacjÍ s≥Ûw, ktÛre sπ w stanie zachowaÊ semantyczne i syntaktyczne powiπzania miÍdzy s≥owami. Przyk≥adem mogπ byÊ tu operacje artymetyczne na wektorach odwzorowujπce semantykÍ s≥Ûw: $vector("krÛl")-vector("mÍøczyzna")+vector("kobieta") \approx vector("krÛlowa")$

Poczπtkowo metody word embedding opiera≥y siÍ w duøej mierze na g≥Íbokich sieciach neuronowych (%TODO podaÊ linki do artyku≥Ûw ) Bengio, Collobert and Weston
Jednak istotnπ wadπ tych rozwiπzaÒ by≥a niska wydajnoúÊ ich nauki, co jest szczegÛlnie odczuwalne w przypadku ogromnych zbiorÛw danych wykorzystywanych w úrodowiskach produkcyjnych. Rozwiπzaniem tego problemu wydajπ siÍ byÊ nowe metody wektorowej reprezentacji s≥Ûw powsta≥e na przestrzeni ostatnich lat. W odrÛønieniu do metod deep learningowych opierajπ siÍ one o metody szybkiej nauki, np. o p≥ytkie sieci neuronowe, ktÛre uczπ siÍ na tyle krÛtko, øe sprawdzajπ siÍ one w zastosowaniach komercyjnych.

WyrÛønia siÍ dwa podstawowe podejúcia generowania wektorowych reprezentacji s≥Ûw:
\begin{enumerate}
...
(Deerwester et al., 1990; Sch¸tze, 1992;
...
\subsection{Word2vec}%TODO obrazek z sieciπ, przepisaÊ

Word2vec jest stosunkowo nowπ (2013) predykcyjnπ metodπ osadzania s≥Ûw w przestrzeni wektorowej, opisanπ w \cite{word2vec}. 
...
\begin{enumerate}
	\item Zgromadü wspÛ≥wystπpienia s≥Ûw w formie macierzy $X$. Kaødy element $X_ij$ takiej macierzy reprezentuje jak czÍsto s≥owo $i$ wystÍpuje w pobliøu s≥owa $j$. Zazwyczaj macierz buduje siÍ poprzez skanowanie bazowego korpusu oknem o ustalonej szerokoúci, w obrÍbie ktÛrego centralne s≥owo leøy w kontekúcie s≥Ûw je otaczajπcych. Dodatkowo moøna tu wprowadziÊ wagi dla s≥Ûw malejπce wraz ze wzrostem dystansu od s≥owa centralnego.
...
%TODO, ale NIE WIEM, CZY MIARA UØYWAJ•CA KEYWORD”W JEST POTRZEBNA
