...
\subsection{\textit{Term frequency --- inverse document frequency}}

\textit{TF-IDF} \cite{tf_idf} (ważenie częstością termów --- odwrotna częstość w dokumentach) jest metodą reprezentacji tekstu jako zbioru słów przy~jednoczesnym uwzględnieniu wagi słów, która zależy od~częstości występowania słowa w~korpusie.
Wartość \textit{TF-IDF} słowa $w_i$ w dokumencie $d_j$ wyraża się wzorem: 
\begin{equation}
...
\subsection{\textit{Distributional semantics}}
...
Celem działania sieci jest maksymalizacja funkcji celu postaci:
\begin{equation}
...
\subsection{\textit{Word2vec}}%TODO przepisać rozdział tak, by był spójny z poprzednimi
...
\begin{itemize}
	\item \textit{CBOW} (\textit{continuous bag-of-words}): na~postawie okna $N$ sąsiednich słów sieć przewiduje słowo, którego z~największym prawdopodobieństwem te $N$ słów jest sąsiedztwem. W~tym modelu funkcja celu przyjmuje postać:
	\begin{equation}
	J_\theta = \frac{1}{T}\sum\limits_{t=1}^T\ \sum\limits_{-n \leq j \leq n , \neq 0} \text{log} \space p(w_{t+j} \: | \: w_t).
	\end{equation}
	\item \textit{skip-gram}: na~podstawie słowa sieć dokonuje predykcji $N$ sąsiednich słów. Zadaniem sieci neuronowej jest wtedy optymalizacja funkcji celu postaci:
	\begin{equation}
...
W~celu dalszego opisu metody \textit{Word2vec} wprowadzam pojęcie funkcji \textit{softmax} użytej w~warstwie wyjściowej sieci neuronowej. \textit{Softmax} jest generalizacją funkcji logistycznej, zamieniającą $K$-wymiarowy wektor $z$ dowolnych liczb rzeczywistych na~$K$-wymiarowy wektor liczb rzeczywistych z~zakresu $(0,1]$, które sumują~się do~$1$. Funkcja wyraża~się wzorem:
\begin{equation}
...
\begin{enumerate}
	...
	\item Zdefiniuj funkcję kosztu:
	\begin{equation}
	J = \sum_{i=1}^V \sum_{j=1}^V \; f(X_{ij}) ( w_i^T w_j + b_i + b_j - \log X_{ij})^2,
	\end{equation}
	gdzie f jest funkcją ważącą, która pomaga zapobiec uczeniu tylko na~podstawie najbardziej popularnych par słów. Autorzy proponują funkcję postaci:
	\begin{equation}
	f(X_{ij}) = 
	\begin{cases}
...
Precyzja to odsetek wyszukanych dokumentów, które są relewantne do~zapytania. 
\begin{equation}
precision=\frac{|\{relevant\} \cap \{retrieved\}|}{|\{retrieved\}|}.
\end{equation}

Zwrot natomiast jest liczbą wyszukanych relewantnych dokumentów w~stosunku do~wszystkich relewantnych dokumentów (również tych niewyszukanych)
\begin{equation}
recall=\frac{|\{relevant\} \cap \{retrieved\}|}{|\{relevant\}|}.
\end{equation}

Bardziej złożoną miarą lepiej charakteryzującą jakość procesu wyszukiwania jest \textit{F-miara}. W~celu obliczenia wyniku uwzględnia ona zarówno precyzję jak i~zwrot. Miara wyraża~się wzorem:
\begin{equation}
...
Miara \textit{nDCG} \cite{ndcg} (\textit{Normalized Discounted Cumulative Gain}) jest miarą jakości rankingu, która opiera~się na~założeniu, że im bardziej relewantne wyniki, tym wyżej powinny być w~rankingu, aby ranking był najbardziej wartościowy. Miara ta mierzy skumulowany ,,zysk'' powstały poprzez umieszczenie poszczególnych przedmiotów na~określonych pozycjach rankingu. Miara \textit{nDCG} jest znormalizowaną wersją miary \textit{DCG} (\textit{Discounted Cumulative Gain}), która wyraża~się wzorem: 
\begin{equation}
{\mathrm  {DCG_{{p}}}}=\sum _{{i=1}}^{{p}}{\frac  {2^{{rel_{{i}}}}-1}{\log _{{2}}(i+1)}},
\end{equation}
gdzie $p$ to liczba elementów rankingu, $i$ to miejsce przedmiotu w~rankingu, a $rel$ to poziom relewantności elementu. \textit{DCG} premiuje relewantne przedmioty, które są wysoko w~rankingu oraz~karze za~relewantne przedmioty w~dole rankingu. W~wariancie \textit{nDCG} następuje jeszcze normalizacja przez~podzielenie wartości \textit{DCG} rzeczywistego rankingu przez~\textit{DCG} idealnego rankingu ($IDCG$) zbudowanego na~elementach korpusu ułożonych malejąco pod~kątem relewantności:
\begin{equation}
{\mathrm  {nDCG_{{p}}}}={\frac  {DCG_{{p}}}{IDCG_{{p}}}},\ \mathrm {IDCG_{p}} =\sum _{i=1}^{|REL|}{\frac {2^{rel_{i}}-1}{\log _{2}(i+1)}},
\end{equation}
...
\begin{enumerate}
	...
	\item Dla~każdej pary obliczam średnią ocen wszystkich $T$ użytkowników:
	\begin{equation}
	avg(j,k)=\frac{1}{T}\sum_{t}^{T}rel_t(b_j, s_{jk}),
	\end{equation}
	gdzie $rel_t$ to ocena $t$-go użytkownika.
	\item Dla~każdego artykułu bazowego $b_j$ obliczam:
	\begin{equation}
...
Dokonuję porównania jakości modelu \textit{LSI} w~zależności od~wartości hiperparametru --- liczby tematów.
...
\begin{table}[H]
	...
	\begin{tabular}{lrrrr}
		...
		\hline
		mut\_cat      & 0.4289 & 0.4383 & 0.4404 & 0.4397 \\
		mut\_cat\_ndcg & 0.5072 & 0.5162 & 0.5181 & 0.5175 \\
		mut\_kw       & 0.2961 & 0.3669 & 0.3980  & 0.3898 \\
		mut\_kw\_ndcg  & 0.0973 & 0.1195 & 0.1291 & 0.1268 \\
		clicks       & 0.9255 & 0.8914 & 0.9132 & 0.9133 \\
		\hline
	\end{tabular}
	\caption{Wyniki ewaluacji metody \textit{LSI} dla zmiennej liczby tematów.}
\end{table}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/results/lsi_.png}
	\caption{Wykres porównujący znormalizowane wyniki dla~metody \textit{LSI}.}
	%\label{fig:warstwy}
\end{figure}

Miary oparte na~liczbie wspólnych słów kluczowych (\textit{mut\_kw...}) wyraźnie pokazują wzrost jakości modelu przy~wzroście liczby tematów. Średnia wszystkich miar pokazuje, że najwyższą jakość model osiąga dla~liczby tematów 500.

\subsection{\textit{LDA} w zależności od liczby tematów}

Podobnie jak w~poprzednim punkcie dokonuję porównania jakości modelu \textit{LDA} w~zależności od~wartości hiperparametru --- liczby tematów.
...
\begin{table}[H]
	...
	\begin{tabular}{lrrrrrr}
		...
		\hline
		mut\_cat      & 0.4114 & 0.4184 & 0.4188 & 0.4210  & 0.4250  & 0.4223 \\
		mut\_cat\_ndcg & 0.4879 & 0.4950  & 0.4942 & 0.4976 & 0.5008 & 0.4989 \\
		mut\_kw       & 0.2196 & 0.2480  & 0.2648 & 0.2819 & 0.2944 & 0.2932 \\
		mut\_kw\_ndcg  & 0.0734 & 0.0815 & 0.0881 & 0.092  & 0.0962 & 0.0964 \\
		clicks       & 0.8809 & 0.9028 & 0.8797 & 0.9064 & 0.9124 & 0.8909 \\
		\hline
	\end{tabular}
	\caption{Wyniki ewaluacji metody \textit{LDA} dla~zmiennej liczby tematów.}
\end{table}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\textwidth]{img/results/lda_.png}
	\caption{Wykres porównujący znormalizowane wyniki dla~metody \textit{LDA}.}
	%\label{fig:warstwy}
\end{figure}

Podobnie jak w~przypadku modelu \textit{LSI} miary oparte na~liczbie wspólnych słów kluczowych (\textit{mut\_kw}) pokazują wzrost jakości modelu przy wzroście liczby tematów. Średnia wszystkich miar pokazuje, że najwyższą jakość model osiąga dla~liczby tematów 900.
...
\begin{table}[H]
	...
	\begin{tabular}{lrr}
		...
		\hline
		mut\_cat      &       0.4267 &      0.4228 \\
		mut\_cat\_ndcg &       0.504  &      0.4968 \\
		mut\_kw       &       0.277  &      0.2784 \\
		mut\_kw\_ndcg  &       0.0896 &      0.0885 \\
		clicks       &       0.9465 &      0.9462 \\
		\hline
	\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}{lrrr}
		...
		\hline
		mut\_cat      &            0.4267 &          0.4044 &       0.3968 \\
		mut\_cat\_ndcg &            0.504  &          0.4797 &       0.5439 \\
		mut\_kw       &            0.277  &          0.2065 &       0.1913 \\
		mut\_kw\_ndcg  &            0.0896 &          0.0686 &       0.0969 \\
		clicks       &            0.9465 &          0.8985 &       0.9967 \\
		\hline
	\end{tabular}
	\caption{Wyniki ewaluacji metody \textit{Word2vec} dla~wektorów dł.~100 w~zależności od~metody porównywania wektorowych reprezentacji dokumentów.}
\end{table}
...
\begin{table}[H]
	...
	\hline
	mut\_cat      &            0.4277 &          0.4072 &       0.3898 \\
	mut\_cat\_ndcg &            0.5055 &          0.4829 &       0.7055 \\
	mut\_kw       &            0.2807 &          0.2124 &       0.1714 \\
	mut\_kw\_ndcg  &            0.0907 &          0.0701 &       0.1200   \\
	clicks       &            0.9364 &          0.8916 &       0.9406 \\
	\hline
	\end{tabular}
	\caption{Wyniki ewaluacji metody \textit{Word2vec} dla~wektorów dł.~300 w~zależności od~metody porównywania wektorowych reprezentacji dokumentów.}
\end{table}
...
Wyniki dla~metody opartej o~tematy \textit{LDA} są jednoznacznie gorsze od~wyników dla~metody centroidu.
...
\begin{table}[H]
	...
	\begin{tabular}{lrrr}
		...
		\hline
		mut\_cat      & 0.4267 & 0.4277 & 0.4272 \\
		mut\_cat\_ndcg & 0.504  & 0.5055 & 0.5046 \\
		mut\_kw       & 0.277  & 0.2807 & 0.2783 \\
		mut\_kw\_ndcg  & 0.0896 & 0.0907 & 0.09   \\
		clicks       & 0.9465 & 0.9364 & 0.939  \\
		\hline
	\end{tabular}
...
\begin{figure}[H]
	...
	\caption{Porównanie znormalizowanych wyników dla metody \textit{Word2vec} w zależności od bazowego korpusu.}
	%\label{fig:warstwy}
\end{figure}

Dla~różnej wymiarowości wektorów wyjściowych metoda \textit{Word2vec} daje bardzo podobne rezultaty. Różnica między~najlepszym i~najgorszym wynikiem wynosi jedynie ok.~1,5\%.  Mimo to najlepszą wartością hiperparametru wydaje~się być 300.
...
\begin{table}[H]
	...
	\begin{tabular}{lrrr}
		...
		długość wektorów &      100 &      300 &     1000 \\
		\hline
		mut\_cat      & 0.4324 & 0.4324 & 0.4432 \\
		mut\_cat\_ndcg & 0.5101 & 0.5101 & 0.5237 \\
		mut\_kw       & 0.2998 & 0.2998 & 0.3586 \\
		mut\_kw\_ndcg  & 0.0971 & 0.0971 & 0.1151 \\
		clicks       & 0.908  & 0.908  & 0.8798 \\
		\hline
	\end{tabular}
...
\begin{figure}[H]
	...
	\caption{Porównanie znormalizowanych wyników dla metody \textit{GloVe} w zależności od bazowego korpusu.}
	%\label{fig:warstwy}
\end{figure}
...
\begin{table}[H]
	...
	\begin{tabular}{lrrr}
		...
		długość wektorów &      100 &      300 &     1000 \\
		\hline
		mut\_cat      & 0.4444 & 0.4466 & 0.4473 \\
		mut\_cat\_ndcg & 0.5252 & 0.5284 & 0.5289 \\
		mut\_kw       & 0.335  & 0.353  & 0.3617 \\
		mut\_kw\_ndcg  & 0.1085 & 0.1139 & 0.1165 \\
		clicks       & 0.8968 & 0.8974 & 0.8856 \\
		\hline
	\end{tabular}
...
\begin{figure}[H]
	...
	\caption{Porównanie znormalizowanych wyników dla metody \textit{FastText} w zależności od bazowego korpusu.}
	%\label{fig:warstwy}
\end{figure}

Również jak w przypadku metody \textit{GloVe} metoda \textit{FastText} osiąga najlepsze rezultaty dla wektorów długości 1000.
...
\begin{table}[H]
	...
	\begin{tabular}{lrrrrrrr}
		...
		\hline
		mut\_cat      &   0.4404 &   0.4250  &   0.4277 &   0.4432 &   0.4473 &    0.4339 &   0.1541 \\
		mut\_cat\_ndcg &   0.5181 &   0.5008 &   0.5055 &   0.5237 &   0.5289 &    0.5091 &   0.1854 \\
		mut\_kw       &   0.3980  &   0.2944 &   0.2807 &   0.3586 &   0.3617 &    0.6175 &   0.0229 \\
		mut\_kw\_ndcg  &   0.1291 &   0.0962 &   0.0907 &   0.1151 &   0.1165 &    0.2027 &   0.0084 \\
		clicks       &   0.9132 &   0.9124 &   0.9364 &   0.8798 &   0.8856 &    0.8904 & nan      \\
		\hline
	\end{tabular}
...
Najwyższe wyniki wszystkich miar uzyskała metoda \textit{allegro}. Jej wysokie wskaźniki dla metody \textit{mut\_kw} wynikają wprost ze struktury zapytania do silnika \textit{Elasticsearch}, które to jest oparte właśnie o słowa kluczowe dołączone do artykułu.
...
\begin{table}[H]
	...
	\begin{tabular}{lrrrrrrr}
		...
		sygnatura metody&   lsi500 &   lda900 &   w2v300 &   gv1000 &   ft1000 &   allegro &     rnd \\
		\hline
		users\_std &   2.6931 &   2.7155 &   2.5982 &   2.1881 &    2.061 &    2.4019 & 1.3401 \\
		\hline
	\end{tabular}
