...
\textbf{SEKCJA DO UZUPE£NIENIA}%TODO
...
W przeprowadzanych testech staram siê dokonaæ porównania pomiêdzy ró¿nymi konfiguracjami tej samej metody oraz pomiêdzy najlepszymi wariantami ró¿nych metod. W tym celu wykorzystujê wprowadzone miary ewaluacji. W wiêkszoœci przypadków u¿ywam miar automatycznych bazuj¹cych na metadanych artyku³ów oraz historycznej aktywnoœci u¿ytkowników serwisu. Do ostatecznego porównania pomiêdzy najlepszymi wariantami testowanych metod stosujê równie¿ miarê opart¹ na ocenie realnych u¿ytkowników. Miara ta daje najbardziej rzetelne wyniki, jednak¿e jej u¿ycie jest wyj¹tkowo kosztowne - wymaga zaanga¿owania osób testuj¹cych oraz sporych nak³adów czasowych. St¹d zdecydowa³em na u¿ycie jej tylko w jednym przypadku.
...
Wykonujê testy adaptacji metod wektorowej reprezentacji s³ów dla wielu konfiguracji. Poni¿ej obszary, na krórych dokonujê zmian konfiguracji.
...
Do wygenerowania wektorów reprezentuj¹cych s³owa u¿ywam metod wektorowej reprezentacji tekstu: word2vec (w2v), GloVe (gv) oraz fastText (ft). Ka¿d¹ z nich testujê pod k¹tem ró¿nej wymiarowoœci generowanych wektorów.
...
\begin{enumerate}
	...
	\item odleg³oœæ cosinusowa pomiêdzy wektorow¹ reprezentacj¹ s³ów kluczowych wyznaczonych metod¹ LDA.
	W tej metodzie u¿ywam metody LDA do wyznaczenia przynale¿noœci  s³ów nale¿¹cych do tematów  LDA %TODO dokoñczyæ
\end{enumerate}
...
W celu posiadania punktu odniesienia dokonujê testów dla metody losowo wybieraj¹cej $n$ podobnych do danego artyku³ów. Wybór nastêpuje zgodnie z rozk³adem jednostajnym ze zbioru wszystkich artyku³ów.

\section{Metody ewaluacji}

W celu wykonania oceny poszczególnych testowanych metod wykorzystujê wszystkie metody opisane w rozdziale 4.
\begin{enumerate}
	\item $clicks$ - ocena na podstawie historycznej aktywnoœci u¿ytkowników mierzona na podstawie liczby klikniêæ w odnoœniki.
	\item $mut_cat[_ndcg]$ - relewantnoœæ wyszukanych artyku³ów na podstawie liczby wspólnych kategorii z artyku³m bazowym. Stosujê dwa warianty: œrednia relewantnoœæ wyszukanych artyku³ów oraz miara nDCG wykorzystuj¹ca miarê tê relewantnoœci.
	\item $mut_kw[_ndcg]$ - relewantnoœæ wyszukanych artyku³ów na podstawie liczby wspólnych s³ów kluczowych z artyku³m bazowym. Równie¿ stosujê dwa warianty: œrednia relewantnoœæ wyszukanych artyku³ów oraz miara nDCG wykorzystuj¹ca miarê tê relewantnoœci.
	\item $users$ - ocena na podstawie eksperckiej oceny u¿ytkowników \textbf{MO¯LIWE, ¯E TU RÓWNIE¯ WARIANT NDCG} %TODO
\end{enumerate}
