...
Obecnie wykorzystywana przez Allegro metoda generowania rekomendacji artyku≥Ûw opiera siÍ o zapytanie do us≥ugi Elasticsearch wykorzystujπce s≥owa kluczowe do≥πczone do artyku≥Ûw. Elasticsearch jest popularnym silnikiem wyszukiwania tekstu opartym o indeks Lucene\cite{lucene}. Dzia≥a w architekturze rozproszonej a komunikacja z nim nastÍpuje poprzez protokÛ≥ HTTP i format JSON. Umoøliwia on efektywne przechowywanie dokumentÛw tekstowych oraz efektywne ich wyszukiwanie. 

Apache Lucene jest bibliotekπ napisanπ w jÍzyku Java s≥uøπcπ do wyszukiwania tekstu, ktÛra w tym celu wykorzystuje mechanizm odwrÛconego indeksu. Zasada dzia≥ania biblioteki polega na stworzeniu s≥ownika ze wszystkich (odpowiednio wstÍpnie przetworzonych) s≥Ûw dokumentÛw przeznaczonych do wyszukiwania. NastÍpnie na bazie Ûw s≥ownika tworzony jest odwrÛcony indeks: kaødemu ze s≥Ûw przypisywana jest lista dokumentÛw, ktÛre zawierajπ to s≥owo. Pozwala to przyspieszyÊ proces wyszukiwania, gdyø w poszukiwaniu pojedynczego s≥owa biblioteka nie przeszukuje ca≥ego zbioru dokumentÛw, a jedynie s≥ownik, ktÛry na ogÛ≥ jest wielokrotnie krÛtszy.

Zaletπ silnika Elasticsearch sπ jego wydajnoúÊ, skalowalnoúÊ, niezawodnoúÊ i prostota uøytkowania, co przek≥ada siÍ na jego duøπ popularnoúÊ wúrÛd np. serwisÛw internetowych\cite{elastic_companies}.

Wadπ metody jest to, øe ogranicza siÍ ona do wyszukiwania tekstowego pomijajπc aspekt semantyczny. Stwarza to trudnoúci przy wyszukiwaniu synonimÛw lub homonimÛw.%Znaczy to, øe jeøeli dwa teksty opisujπ ten sam temat, ale uøywajπ to tego rÛønych s≥Ûw, np. synonimÛw, to systemowi opartemu jedynie o wyszukiwanie tekstowe nie uda siÍ stwierdziÊ podobieÒstwa miÍdzy tymi tekstami, mimo, iø takowe istnieje. Innπ trudnoúciπ jest problem polisemii - sytuacji, gdy jedno s≥owo posiada dwa znaczenia (homonimy). Metoda  wyszuka wszystkie dokumenty je zawierajπce niezaleønie od kontekstu, w jakim szuka ich uøytkownik.
...
Bag-of-words (worek s≥Ûw)\cite{bow} jest jednπ z pierwszych koncepcji reprezentacji tekstu jako zbioru zawartych w nim s≥Ûw w postaci wektorÛw. Metoda nie zachowuje kolejnoúci s≥Ûw w tekúcie, lecz liczbÍ ich wystπpieÒ. Istotnπ zaletπ reprezentacji wektorowej dokumentÛw jest moøliwoúÊ zdefiniowania miary odleg≥oúci pomiÍdzy dokumentami (np. miara kosunusowa opisana pÛüniej) odzwierciedlajπcej ich podobieÒstwo. TechnikÍ tÍ moøna opisaÊ jako przekszta≥cenie z korpusu w przestrzeÒ wektorÛw $ bow: C \to \mathbb{R}^n $ gdzie:\\
$C$: korpus\\
...
TF-IDF\cite{tf_idf} (waøenie czÍstoúciπ termÛw - odwrotna czÍstoúÊ w dokumentach) jest metodπ reprezentacji tekstu jako zbioru s≥Ûw przy jednoczesnym uwzglÍdnieniu wagi s≥Ûw, ktÛra zaleøy od czÍstoúci wystÍpowania s≥owa w korpusie.  Oznaczenia formalne takie same tak w przypdku BOW.
...
Dokumenty reprezentowane sπ tu jako wektory, sk≥adajπce siÍ z wag s≥Ûw wystÍpujπcych w kaødym z nich. TF-IDF przechowuje informacjÍ o czÍstotliwoúci wystÍpowania s≥Ûw biorπc przy tym pod uwagÍ istotnoúÊ znaczenia s≥owa lokalnego w stosunku do jego znaczenia w kontekúcie ca≥ego zbioru dokumentÛw. W tej technice s≥owa wystÍpujπce rzadko sπ premiowane wzglÍdem s≥Ûw pospolitych. Wadπ metody tej i poprzedniej jest postaÊ wektorÛw reprezentujπcych s≥owa: sπ to na ogÛ≥ rzadkie wektory duøej wymiarowoúci.
...
LDA jest technikπ automatycznego wykrywania tematÛw zawartych w dokumentach.

LDA reprezentuje dokumenty jako mieszanki tematÛw, ktÛre z kolei sk≥adajπ sie z mieszanki s≥Ûw ca≥ego korpusu.

Wybierz mieszankÍ tematÛw dla dokumentu (zgodnie z rozk≥adem Dirichleta dla ustalonej liczby $K$ tematÛw)

	Wybierz temat zgodnie z rozk≥adem wielomianowym.
	
	Uøyj tematu do wybrania s≥owa zgodnie z rozk≥adem wielomianowym.
	

LDA iteruje po dokumentach starajπc siÍ znaleüÊ mieszankÍ tematÛw, ktÛre z najwiÍkszym prawdopodobieÒstwem wygenerowa≥y zbiÛr s≥Ûw dokumentu.
...
Jego ideπ jest reprezentacja tekstÛw ja-
ko mieszanki tematÛw, definiowanych jako rozk≥ady prawdopodobieÒstwa na zbiorze s≥Ûw.
Z jednej strony jest to pewna metoda redukcji wymiaru danych, natomiast z drugiej strony
LDA jest narzÍdziem, ktÛre dostarcza uøytecznej reprezentacji danych wraz z czÍsto cennπ
interpretacjπ.


Model LDA jest przyk≥adem probabilistycznego modelu graficznego, a konkretniej sieci bay-
esowskiej, ktÛry zosta≥ zaproponowany do modelowania danych tekstowych w [5]. Ideπ LDA
jest reprezentacja tekstu jako mieszanki tematÛw, definiowanych jako rozk≥ady prawdopo-
dobieÒstwa na zbiorze s≥Ûw. W modelu zak≥ada siÍ, øe kaøde s≥owo w dokumencie pochodzi
z pewnego tematu i ostatecznie staramy siÍ opisaÊ dokument jako procentowy rozk≥ad za-
wartych w nim tematÛw. LDA jest metodπ uczenia bez nadzoru i moøna jπ traktowaÊ jako
pewien sposÛb redukcji wymiaru, gdyø staramy siÍ przedstawiÊ zbiÛr tekstÛw, przy pomo-
cy pewnej (mniejszej niø liczba tekstÛw) liczby tematÛw. Ponadto z praktycznego punktu
widzenia czÍsto waøna jest rÛwnieø interpretacja wyestymowanych tematÛw.

Przedstawimy ideÍ modelu LDA na przyk≥adzie. Za≥Ûømy, øe naszymi dokumentami sπ trzy
zdania:
ï Mam gorπczkÍ i katar;
ï Graliúmy w siatkÛwkÍ;
ï Sport to zdrowie;2.3. Latent Dirichlet Allocation
19
gdzie pogrubienie oznacza, øe tylko te s≥owa pozosta≥y w tekstach po wstÍpnym przygoto-
waniu danych. Za≥Ûømy rÛwnieø, øe mamy okreúlone dwa tematy, z czego w pierwszym naj-
czÍstsze s≥owa to ÑprzeziÍbienieî, Ñgorπczkaî, Ñgrypaî i inne s≥owa zwiπzane ze zdrowiem,
natomiast w drugim sπ to Ñpi≥kaî, ÑgraÊî, Ñsportî i inne zwiπzane ze sportem. WÛwczas
jeøeli zak≥adamy, øe s≥owa w dokumentach pochodzπ z jakiegoú tematu, to moøna intuicyjnie
oceniÊ, øe pierwsze zdanie w 100% bÍdzie sk≥adaÊ siÍ ze s≥Ûw z tematu Ñzdrowieî, drugie
w 100% z tematu Ñsportî, a trzecie w 50% z obu tematÛw.
W niniejszej pracy opisujemy model LDA w zastosowaniu do modelowania danych teksto-
wych. Do formalnego opisu modelu, bÍdzie potrzebne kilka pojÍÊ, ktÛre teraz zdefiniuje-
my:
ï S≥owo ñ podstawowa jednostka danych. ZbiÛr wszystkich s≥Ûw nazywamy Ñs≥ownikiemî,
a jego licznoúÊ oznaczamy przez V . Formalnie s≥owo reprezentujemy jako liczbÍ ze zbioru
{1, ..., V }.
ï Dokument ñ skoÒczony ciπg s≥Ûw dowolnej d≥ugoúci. Formalnie jest ciπg liczbowy o war-
toúciach ze zbioru {1, ..., V }, ktÛry moøna interpretowaÊ jako wektor.
ï Korpus ñ zbiÛr sk≥adajπcy siÍ ze skoÒczonej liczby dokumentÛw.
ï Temat ñ dyskretny rozk≥ad prawdopodobieÒstwa wymiaru V , opisujπcy rozk≥ad na zbio-
rze s≥Ûw. W pracy bÍdziemy wymiennie uøywaÊ sformu≥owaÒ Ñrozk≥ad s≥Ûw w temacieî,
Ñrozk≥ad tematuî

W modelu LDA wystÍpujπ trzy parametry:
ï K ñ liczba tematÛw,
ï ? ñ wektor dodatnich liczb rzeczywistych, sterujπcy rozk≥adami tematÛw w dokumen-
tach,
ï ? ñ macierz wymiaru K ◊ V , ktÛrej i-ty wiersz opisuje rozk≥ad i-tego tematu.
Jedynym parametrem, ktÛrego wartoúÊ trzeba zadaÊ, jest K. Oznacza to, øe musimy a priori
za≥oøyÊ, ile tematÛw w danych chcemy wykryÊ. Wartoúci ? oraz ? moøna ustaliÊ z gÛry lub
wyestymowaÊ ich wartoúci, w zaleønoúci od problemu, do ktÛrego wykorzystujemy model.
LiczbÍ dokumentÛw w korpusie bÍdziemy oznaczaÊ jako M .
...
LDA\cite{lda} jest modelem, w ktÛrym kaødy dokument jest modelowany przez skoÒczony zbiÛr tematÛw, z ktÛrych siÍ sk≥ada. Kaødy temat z kolei
...
Celem dzia≥ania sieci jest maksymalizacja funkcji celu $J_\theta = \frac{1}{T}\sum\limits_{t=1}^T\ \text{log} \space f(w_t , w_{t-1} , \cdots , w_{t-n+1})$, gdzie $f(w_t , w_{t-1} , \cdots , w_{t-n+1})$ odpowiada prawdopodobieÒstwu $p(w_t \: | \: w_{t-1} , \cdots , w_{t-n+1})$ wystπpienia s≥owa $w_t$ bezpoúrednio po sekwencji s≥Ûw $w_{t-1} , \cdots , w_{t-n+1}$. Wektorowa reprezentacja s≥owa uzyskiwana jest tu przez przemnoøenie wejúciowego wektora (wektor zer z jedynkπ na $i$-tym miejscu reprezentujπcy $i$-te s≥owo, ,,one-hot-vector'') z macierzπ wag pierwszej warstwy sieci.


Podejúcie to jak i kolejne (\cite{c_w}) wykorzystujπce g≥Íbokie sieci neuronowe nie znalaz≥y zastosowaÒ komercyjnych, poniewaø ich wydajnoúÊ nauki jest na tyle  niska, øe niemoøliwe jest uøycie przy ogromnych zbiorach danych wykorzystywanych w úrodowiskach produkcyjnych.
...
%TODO OpisaÊ co to jest n-gram, word2vec ma≥π literπ
\subsection{FastText}

FastText\cite{fasttext} to biblioteka stworzona w 2016 w celu wydajnego uczenia wektorowej reprezentacji s≥Ûw oraz klasyfikacji zdaÒ. Od ,,klasycznego'' word2vec rÛøni siÍ stopniem szczegÛ≥owoúci analizy s≥Ûw. Word2vec traktuje s≥owo jakπ najmniejszπ, niepodzielnπ jednostkÍ, ktÛrej wektorowπ reprezentacjÍ usi≥uje wyznaczyÊ. FastText natomiast dokonuje analizy rÛwnieø wewnÍtrznej analizy s≥Ûw. Wykorzystuje w tym celu rozbicie s≥owa na pods≥owa - ciπgi znakÛw o okreúlonej d≥ugoúci $n$, ,,character n-grams''. Np. s≥owo ,,pokÛj'' sk≥ada siÍ nastÍpujπcych 3-gramÛw: [pok], [okÛ], [kÛj]. Podejúcie takie daje szereg nowych moøliwoúci. Pomaga wyznaczyÊ reprezentacjÍ wektorowπ rzadkich s≥Ûw, ktÛre byÊ moøe majπ wspÛlny rdzeÒ (i znaczenie) z innymi, czÍúciej wystÍpujπcymi s≥owami. Metoda pozwala rÛwnieø nadaÊ wektorowπ reprezentacjÍ s≥owom, ktÛrych w ogÛle nie ma w s≥owniku, jako øe ich pods≥owa mogπ naleøeÊ do s≥Ûw w s≥owniku siÍ znajdujπcych. Zalety te wydajπ siÍ byÊ szczegÛlnie obiecujπce w przypadku bogatych morfologicznie jÍzykÛw, np. jÍzyka polskiego, tureskiego, czy fiÒskiego.

Zasada dzia≥ania metody bazuje na word2vec. Jednakøe oprÛcz predykcji tylko ca≥ych s≥Ûw nastÍpuje tu rÛwnieø predykcja n-gramÛw s≥owa $a$ w otoczeniu s≥owa $a$. Ostatecznie s≥owu $a$ zostaje przypisany wektor sk≥adajπcy siÍ ze úredniej oryginalnej reprezentacji wektorowej s≥owa oraz reprezentacji jego n-gramÛw.
