...
\begin{enumerate}
	...
	\item $users$ - ocena na podstawie eksperckiej oceny u¿ytkowników. W badaniu wykorzysta³em 5 u¿ytkowników operuj¹cych ka¿dy na tym samym zbiorze par testowych. Pary zosta³y wygenerowane (zgodnie z wczeœniejszym opisem metody) na podstawie 50 artyku³ów bazowych wylosowanych spoœród wszystkich artyku³ów udostêpnionych mi przez Allegro.
\end{enumerate}
...
W rozdziale tym przedstawiam wyniki oceny jakoœci poszczególnych metod w zale¿noœci od ich hiperparametrów. Oceny dokonujê na podstawie opisanych uprzednio miar. Do wyników ka¿dego testu do³¹czam ich interpretacjê oraz wnioski.
...
Miary oparte na liczbie wspólnych s³ów kluczowych ($mut\_kw...$) wyraŸnie pokazuj¹ wzrost jakoœci modelu przy wzroœcie liczby tematów. Œrednia ze wszystkich miar pokazuje, ¿e najwy¿sz¹ jakoœæ model osi¹ga dla liczby tematów 500.
...
Podobnie jak w przypadku modelu LSI miary oparte na liczbie wspólnych s³ów kluczowych ($mut_kw$) pokazuj¹ wzrost jakoœci modelu przy wzroœcie liczby tematów. Œrednia ze wszystkich miar pokazuje, ¿e najwy¿sz¹ jakoœæ model osi¹ga dla liczby tematów 900.
...
Maksymalna ró¿nica miêdzy najlepszym i najgorszym wynikiem nie przekracza 2\%. St¹d trudno jest stwierdziæ, która metoda daje tu lepsze wyniki. Na korzyœæ modelu word2vec uczonego na korpusie artyku³ów przemawia jego ma³y rozmiar, a co za tym idzie szybkoœæ nauki. Z przyczyn braku wyraŸnych zalet korzystania tu z modelu uczonego na korpusie s³owosieci korzystam dalej jedynie z modeli opartych o korpus artyku³ów Allegro.

\section{Word2vec w zale¿noœci od metody porównywania dokumentów}

W niniejszym punkcie dokonujê porównania jakoœci metod okreœlania podobieñstwa miêdzy wektorowymi reprezentacjami dokumentów tekstowych wyznaczonymi metod¹ word2vec dla wektorów d³ugoœci 100 oraz 300. Biorê tu pod uwagê metody: centroidu ca³oœci dokumentu ($w2v100_centroid$), centroidu s³ów kluczowych wyznaczonych metod¹ LDA dla 100 tematów ($w2v100_lda100$) oraz Word Mover's Distance ($w2v100_wnd$).
...
Dla obu konfiguracji metody word2vec metoda Word Mover's Distance wykazuje siê najwiêksz¹ zdolnoœci¹ do poprawnego szeregowania dokumentów (wysoki wskaŸnik metod nDCG w porównaniu z metodami wykorzystuj¹cymi œredni¹).
...
W niniejszym punkcie dokonujê porównania najlepszych konfiguracji testowanych wczeœniej metod. Zestawiam z nimi wyniki dla metody u¿ywanej dotyczczas w Allegro oraz metodê losow¹.
Testowane warianty:
\begin{itemize}
	\item $lsi500$ --- Latent Semantic Indexing dla 500 tematów,
	\item $lda900$ --- Latent Dirichlet Allocation dla 900 tematów,
	\item $w2v300$ --- word2vec dla wektorów d³. 300 i przy u¿ytej metodzie centroidu,
	\item $w2v300$ --- GloVe dla wektorów d³. 1000 i przy u¿ytej metodzie centroidu,
	\item $w2v300$ --- fastText dla wektorów d³. 1000 i przy u¿ytej metodzie centroidu,
	\item $allegro$ --- u¿ywane dotychczas w Allegro zapytanie do silnika elasticsearch,
	\item $rnd$ --- metoda losowa.
\end{itemize}
\subsection{Zestawienie wyników testów automatycznych}
Poni¿ej zestawienie wyników miar automatycznych dla najlepszych wariantów testowanych wczeœniej metod.
\begin{center}
...
Wszystkie testowane metody daj¹ du¿o lepsze rezultaty od metody losowej. Stasunkowo wysokie wskaŸniki $mut\_cat$ dla metody losowej wynikaj¹ z du¿o wiêkszej szansy, ¿e dwa artyku³y maj¹ wspólnych przodków w drzewie kategorii ni¿, ¿e maj¹ wspólne s³owa kluczowe.

Najwy¿sze wyniki wszystkich miar uzyska³a metoda $allegro$. Jej wysokie wskaŸniki dla metody $mut_kw$ wynikaj¹ wprost ze struktury zapytania do silnika elasticsearch, które to jest oparte w³aœnie o s³owa kluczowe do³¹czone do artyku³u.
...
%TODO dodatkowe statystyki userskie
...
\begin{itemize}
	\item Gensim --- rozbudowana biblioteka s³u¿¹ca do przetwarzania jêzyka naturalnego; zawiera implementacjê metod word2Vec, LSI, LDA, TF-IDF i inne.
